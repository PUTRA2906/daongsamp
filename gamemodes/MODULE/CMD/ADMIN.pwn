CMD:acmds(playerid)
{
	...
	/reports /ar /dr /vmodels /vehname /apv /aveh /gotoveh /getveh /respawnveh /respawnrad /gotodealer /gotovending");
	...
	/createdealer /createvending /editdealer /editvending /deletedealer /deletevending");
 	
	strcat(line3, "\n"BLUE_E"IL:RP "WHITE_E"- Anti-Cheat is actived.\n\
...
CMD:hcmds(playerid)
{
	...
	strcat(line3, "\n"BLUE_E"FI:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"SoI:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:a(playerid, params[])
{
	...
	foreach(new ii : Player)
	{
		...
		{
			SendClientMessage(ii, 0x99cc00FF, mstr);
		}
	}
	if(i != -1)
	{
		foreach(new ii : Player)
		{
			if(pData[ii][pAdmin] > 0)
			{
				SendClientMessage(ii, 0x99cc00FF, line);
			}
		}
	}
	return true;
...
CMD:goto(playerid, params[])
{
	...
	SendAdminMessage(COLOR_RED, "%s has teleport to player ID: %d.", pData[playerid][pAdminname], otherid);
...
CMD:gethere(playerid, params[])
{
    ...
    SendAdminMessage(COLOR_RED, "%s has get player ID: %d.", pData[playerid][pAdminname], otherid);
    return 1;
}
...
CMD:setjob1(playerid, params[])
{
	...
	if(jobid < 0 || jobid > 9)
        return Error(playerid, "Invalid ID. 0 - 9.");
...
CMD:setvip(playerid, params[])
{
	...
	else
	{
		...
		SendAdminMessage(COLOR_RED, "Server: "GREY2_E"Admin %s(%d) telah menset VIP kepada %s(%d) selama %d hari ke level %s!", pData[playerid][pAdminname], playerid, pData[otherid][pName], otherid, dayz, GetVipRank(otherid));
	}
...
CMD:setstock(playerid, params[])
{
	...
	if(sscanf(params, "s[64]S()[128]", name, string))
    {
        ...
        SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} [material], [component], [product], [gasoil], [food], [cargomeat], [cargoseed]");
        return 1;
    }
    ...
    else if(!strcmp(name, "cargomeat", true))
    {
		...
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set cargomeat stok to %d.", pData[playerid][pAdminname], stok);
    }
    else if(!strcmp(name, "cargoseed", true))
    {
		...
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set cargoseed stok to %d.", pData[playerid][pAdminname], stok);
    }
	return 1;
}
...
function WhitelistPlayer(adminid, NameToJail[])
{
	...
	else
	{
		...
		foreach(new i : Player)
		{
		    if(pData[i][pAdmin] > 0)
		    {
		        SendClientMessageEx(i, COLOR_RED, "Server: "GREY2_E"Admin %s telah men-whitelist player %s", pData[adminid][pAdminname], NameToJail);
		    }
		}
		new query[512];
