...
Bisnis_Type(bisid)
{
    ...
    if(bData[bisid][bType] == 5) // Bar
	{
	    switch(random(2))
		{
			case 0:
			{
				bData[bisid][bIntposX] = 501.9877;
				bData[bisid][bIntposY] = -67.6221;
				bData[bisid][bIntposZ] = 998.7578;
				bData[bisid][bIntposA] = 182.1364;
				bData[bisid][bInt] = 11;
			}
			case 1:
			{
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		{
			bData[id][bMap] = CreateDynamicMapIcon(bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 49, -1, -1, -1, -1, 70.0);
		}
		else
		{
			DestroyDynamicMapIcon(bData[id][bMap]);
...
CMD:createbisnis(playerid, params[])
{
	...
	new price, type;
	if(sscanf(params, "dd", price, type)) return Usage(playerid, "/createbisnis [price] [type, 1.Fastfood 2.Market 3.Clothes 4.Equipment 5.Bar]");
...
CMD:editbisnis(playerid, params[])
{
	...
	else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", btype))
            return Usage(playerid, "/editbisnis [id] [Type] [1.Fastfood 2.Market 3.Clothes 4.Equipment 5.Bar]");
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "{FF0000}FI:RP {0000FF}Bisnis", CMDSString, "Select", "Cancel");
	return 1;
}
