...
CMD:service(playerid, params[])
{
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
		...
		if(GetNearestVehicleToPlayer(playerid, 3.5, false) == pData[playerid][pMechVeh])
		{
			...
			if(pData[playerid][pWsEmplooye] != -1)
			{
				format(Dstring, sizeof(Dstring), "Service Name\tComponent\n\
				Engine Fix\t%d\n", engine);
				format(Dstring, sizeof(Dstring), "%sBody Fix\t%d\n", Dstring, body);
				format(Dstring, sizeof(Dstring), "%sTires Fix\t%d\n", Dstring, velg);
				format(Dstring, sizeof(Dstring), "%sSpray Car\t40\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sPaint Job Car\t100\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sWheels Car\t85\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sSpoiler Car\t70\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sHood Car\t70\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sVents Car\t70\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sLights Car\t70\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sExhausts\t80\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sFront bumpers\t100\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sRear Bumpers\t100\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sRoofs Car\t70\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sSide skirts\t90\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sBullbars\t50\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sStereo\t150\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sHydraulics\t150\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sNitro 1\t250\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sNitro 2\t375\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sNitro 3\t500\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sNeon\t450\n", Dstring);
...
CMD:hydraulic(playerid)
{
	if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
	    for(new idx = 0; idx < 6; idx ++)
		{
		    if(IsPlayerInRangeOfPoint(playerid, 5.0, h_hydr[idx][H_BAWAH][0], h_hydr[idx][H_BAWAH][1], h_hydr[idx][H_BAWAH][2]))
			{
				MoveDynamicObject(hydr[idx],  h_hydr[idx][H_ATAS][0], h_hydr[idx][H_ATAS][1], h_hydr[idx][H_ATAS][2], 1);
				PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
			}
		}
	}
	else Error(playerid, "Anda bukan pekerja mechanic!");
	
	return 1;
}
CMD:dhydraulic(playerid)
{
    if(pData[playerid][pJob] == 2 || pData[playerid][pJob2] == 2)
	{
	    for(new idx = 0; idx < 6; idx ++)
		{
		    if(IsPlayerInRangeOfPoint(playerid, 5.0, h_hydr[idx][H_BAWAH][0], h_hydr[idx][H_BAWAH][1], h_hydr[idx][H_BAWAH][2]))
			{
				MoveDynamicObject(hydr[idx], h_hydr[idx][H_BAWAH][0], h_hydr[idx][H_BAWAH][1], h_hydr[idx][H_BAWAH][2], 1);
	   			PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
			}
		}
	}
